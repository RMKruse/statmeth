...
(b)
Wir bauen ein Measurement Modell, welche erst einmal aus den erklärenden Variablen $x_1, x_2$ und $x_3$ eine neue latente Größe erstellt, welche wir nutzen um die Variable `y1` zu erklären.
Erst einmal nehmen wir keine speziellen Annahmen an über die Correlationen. 
Kommt dabei das gleiche Ergebnis raus, wenn wir die Variable `y1` direkt durch $x_1, x_2$ und $x_3$ erklären?
(4 Punkte)
...
**Antwort:** Nein, es wird nicht das gleiche Modell (Ergebnis) dabei herauskommen, da das Measurement Modell eine Form der Faktoranalyse macht, und die X Variablen nicht zu 100 genutzt werden.

(c)
Erstelle nun ein Modell welche die Größen $y5 + y6 + y7 + y8$ nutzt um eine weitere neue Größe `Var3` zu bemessen.
...
Welches dieser beiden Modelle ist das beste? Begründe.
(8 Punkte)
...
m_gleichung <- 'Var1 =~ x1 + x2 + x3
                Var2 =~ y1 + y2 + y3 
                Var3 ~ Var1 + Var2' 
m_c1 <- sem(m_gleichung, data=datensatz2)
